...
#out_of_vocab_wordvec = [np.random.random((1,ndims)) for i in range(8)]  #THIS is an assumption, may deteriorate accuracy
...
def get_similarity(q1, q2, print_out=False):
    ...
    similarity_wmd = activation(1/(word_mov_distance(q1,q2)+1e-10)) #((-np.log(sigmoid(wmd(q1,q2)+4)))#activation calculate log(1/x) since curve is a better representation 
    ...
    try:
        if(word_vectors_loaded):
            average_similarity = 0.43*similarity_wmd + 0.42*similarity_wordnet + 0.15*similarity_w2v #+ 0.8*similarity_siamese #Restricting between 0 and 1 doesn't matter as final result is comparitive but is helpful for evaluation of individual functions
        else:
...
def most_similar_to_q(q1, k=5, need_qs=False):
    ...
    for sim_id, sim_score in similarity_scores:
        ...
        print("Q_id:{}\n --------------------------\n {} \n Similarity score:{}".format(sim_id,all_existing_questions_list[sim_id],sim_score))
        if(sim_score > 0.9):
